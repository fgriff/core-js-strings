...
function removeFirstOccurrences(str, value) {
  const startIdx = str.indexOf(value);
  const endIdx = startIdx + value.length;

  return startIdx > -1 ? str.slice(0, startIdx) + str.slice(endIdx) : str;
}
...
function removeLastOccurrences(str, value) {
  const startIdx = str.lastIndexOf(value);
  const endIdx = startIdx + value.length;
...
function findLongestWord(sentence) {
  ...
  let maxLength = 0;
  let firstLongestWordIdx = 0;

  words.forEach((word, idx) => {
    if (maxLength < word.length) {
      maxLength = word.length;
      firstLongestWordIdx = idx;
    }
  });

  return words[firstLongestWordIdx];
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (/[A-Z]/.test(str[i])) {
      invertedCaseStr += str[i].toLowerCase();
    } else if (/[a-z]/.test(str[i])) {
      invertedCaseStr += str[i].toUpperCase();
    } else {
      invertedCaseStr += str[i];
    }
  }

  return invertedCaseStr;
}
...
function extractNameFromTemplate(value) {
  return /(?<=Hello, )(?<name>[a-z ]+)/i.exec(value).groups.name;
}
...
function encodeToRot13(str) {
  const rotate = 13;
  const upperCaseMiddle =
    'A'.charCodeAt() + Math.floor(('Z'.charCodeAt() - 'A'.charCodeAt()) / 2);
  const lowerCaseMiddle =
    'a'.charCodeAt() + Math.floor(('z'.charCodeAt() - 'a'.charCodeAt()) / 2);
  let encodedStr = '';
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (/[A-Z]/.test(str[i])) {
      if (code <= upperCaseMiddle) {
        encodedStr += String.fromCodePoint(code + rotate);
      } else {
        encodedStr += String.fromCodePoint(code - rotate);
      }
    } else if (/[a-z]/.test(str[i])) {
      if (code <= lowerCaseMiddle) {
        encodedStr += String.fromCodePoint(code + rotate);
      } else {
        encodedStr += String.fromCodePoint(code - rotate);
      }
